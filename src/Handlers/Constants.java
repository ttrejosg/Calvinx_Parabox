package Handlers;

/**
 * Clase que contiene todas las constantes utilizadas en el juego.
 * @author Tomás y Carlos
 */
public class Constants {
    public static final int GAME_WIDTH = 1280;
    public static final int GAME_HEIGHT = 720;
    public static final int FPS_SET = 120;
    public static final int UPS_SET = 200;
    public static final int ANY_SPEED = 25;
    public static final int BLOCKS_SIZE = 60;
    public static final String ROOMS_PATH = "src/Rooms";
    public static final String FLOOR_PATH = "floor.png";
    public static final String BACKGROUND_PATH = "Background_Levels.png";
    public static final String PAUSE_BACKGROUND_PATH = "Pause.png";
    public static final String DOWN_PATH = "down.png"; // _
    public static final String UP_PATH = "up.png"; // -
    public static final String LEFT_PATH = "left.png"; // [
    public static final String RIGHT_PATH = "right.png"; // ]
    public static final String DLCORNER_PATH = "dl_corner.png"; // 1
    public static final String DRCORNER_PATH = "dr_corner.png"; // 2
    public static final String ULCORNER_PATH = "ul_corner.png"; // 3
    public static final String URCORNER_PATH = "ur_corner.png"; // 4
    public static final String INDLCORNER_PATH = "in_dl_corner.png"; // 5
    public static final String INDRCORNER_PATH = "in_dr_corner.png"; // 6
    public static final String INULCORNER_PATH = "in_ul_corner.png"; // 7
    public static final String INURCORNER_PATH = "in_ur_corner.png"; // 8
    public static final String UDOWN_PATH = "u_down.png"; // *
    public static final String UUP_PATH = "u_up.png"; // ^
    public static final String ULEFT_PATH = "u_left.png"; // +
    public static final String URIGHT_PATH = "u_right.png"; // %
    public static final String TUNELVERTICAL_PATH = "tunel_vertical.png"; // v
    public static final String TUNELHORIZONTAL_PATH = "tunel_horizontal.png"; // h
    public static final String RELEASEZONE_PATH = "releaseZone.png"; // r
    public static final String DETOURRIGHT_PATH = "detour_right.png"; // i
    public static final String BLOCK_PATH = "block_sprites.png"; // b
    public static final String DOOR_PATH = "door.png"; // d
    public static final String TP_PATH = "Tp.png"; // t
    public static final String ENEMY_PATH = "enemy_sprites.png"; // e
}

